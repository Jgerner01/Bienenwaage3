...
void ButtonHandler::begin() {
    // GPIO36/39 sind Input-Only ohne internen Pull-Up → externe Widerstände
        ...
        case BtnState::PRESSED:
            if (!pressed) {
                if (millis() - _btn1PressMs >= BTN_DEBOUNCE_MS) {
                    // Kurzdruck: nächstes aktives Modul
                    uint8_t count = hx711.getModuleCount();
                    uint8_t next  = _selectedModule;
                    for (uint8_t i = 1; i <= count; i++) {
                        uint8_t candidate = (_selectedModule + i) % count;
                        if (hx711.getModule(candidate).active) {
                            next = candidate;
                            break;
                        }
                    }
        ...
        case BtnState::PRESSED:
            if (!pressed) {
                ...
                if (held >= BTN_LONG_PRESS_MS) {
                    // Langdruck: Ertragstara-Bestätigung anfordern
                    ...
                    // Kurzdruck: Schnellmessung starten
